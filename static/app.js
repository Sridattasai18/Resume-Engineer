        ...
        function saveKey() {
            ...
            try {
                if (k) {
                    localStorage.setItem('re-apikey-' + selProvider, k);
                } else {
                    localStorage.removeItem('re-apikey-' + selProvider);
                }
            } catch (e) { }
        ...
        // Backend PDF upload selection reference
        var uploadedPDFFile = null;
        ...
        async function handlePDFUpload(input) {
            ...
            if (status) {
                ...
                status.textContent = '📄 ' + file.name + ' selected. PDF text will be parsed by server.';
            }
        ...
        function loadKey() {
            try {
                ...
                if (!k && selProvider === 'gemini') {
                    document.getElementById('api-key-input').placeholder = 'Using server default…';
                } else {
                    document.getElementById('api-key-input').placeholder = 'Paste your key here…';
                }
            } catch (e) { }
        }
        ...
        async function generate() {
            ...
            if (!resume && !uploadedPDFFile) { alert(isL ? 'Please paste your LaTeX resume or upload a PDF.' : 'Please fill in at least your basic details.'); return; }
            ...
            if (!apiKey && selProvider !== 'gemini') { alert('API key required. Click the Keys button in the nav and paste your key.'); return; }
            ...
            // Prepare FormData to send to Flask backend /api/generate
            var formData = new FormData();
            formData.append('provider', selProvider);
            formData.append('model', selModel);
            formData.append('apiKey', apiKey);
            formData.append('jd', jd);
            formData.append('hint', hint);
            formData.append('wantCover', wC ? 'true' : 'false');
            formData.append('wantDiff', wD ? 'true' : 'false');
            formData.append('mode', mode);

            if (isPDF) {
                formData.append('pdf_resume', uploadedPDFFile);
            } else {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    throw new Error(errData.error || ('API generation failed with status ' + res.status));
                }
